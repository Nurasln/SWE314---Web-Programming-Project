...
import { Plus, Trash2, UserCog, ShieldCheck, ChefHat, AlertCircle } from 'lucide-react';
...
const StaffManagement = () => {
  ...
  const canManageStaff = currentRole === 'admin' || currentRole === 'manager';
  ...
  const fetchStaff = async () => {
    try {
      ...
      setError(error.response?.data?.detail || 'Failed to fetch staff.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddStaff = async (e) => {
    ...
    try {
      ...
      setError(error.response?.data?.detail || 'Failed to add staff.');
    }
  };

  const handleDeleteStaff = async (id) => {
    if (!canDeleteStaff) {
      setError('Only admins can delete staff members.');
      return;
    }
    ...
    if (window.confirm('Are you sure you want to remove this staff member?')) {
      try {
        ...
        setError(error.response?.data?.detail || 'Failed to delete staff.');
      }
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">
            Manage restaurant roles, access pins, and authorization levels.
          </p>
          <p className="text-xs text-gray-400 mt-2">
            Current role: <span className="font-bold uppercase">{currentRole || 'unknown'}</span>
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
          ...
          <p className="text-sm mt-1">Full access, can delete staff.</p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
          <p className="text-xs text-gray-400 font-bold uppercase">Manager</p>
          <p className="text-sm mt-1">Can view and add staff.</p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
          <p className="text-xs text-gray-400 font-bold uppercase">Waiter</p>
          <p className="text-sm mt-1">Limited operational access.</p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
          <p className="text-xs text-gray-400 font-bold uppercase">Chef</p>
          <p className="text-sm mt-1">Kitchen-focused limited access.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          staff.map((member) => (
            <div
              key={member.id}
              className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 relative group flex flex-col"
            >
              {canDeleteStaff && (
                <button
