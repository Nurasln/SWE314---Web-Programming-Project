...
from schemas import AISuggestionRequest, AISuggestionResponse, CategoryCreate, StaffCreate, DashboardStats, PaymentRequest
...
# Table Endpoints
# -----------------
@app.post("/tables/{table_id}/sync-item")
def sync_table_item(table_id: int, menu_item_id: int, delta: int, session: Session = Depends(get_session)):
    # 1. Find or create pending order
    order = session.exec(
        select(Order).where(Order.table_id == table_id, Order.status == OrderStatus.pending)
    ).first()
    ...
    if order_item:
        ...
        order_item = OrderItem(order_id=order.id, menu_item_id=menu_item_id, quantity=delta)
        session.add(order_item)
...
@app.get("/tables/{table_id}/active-order", response_model=Optional[Order])
def get_active_table_order(table_id: int, session: Session = Depends(get_session)):
    return session.exec(
        select(Order).where(Order.table_id == table_id, Order.status == OrderStatus.pending)
    ).first()
...
def get_order_details(order_id: int, session: Session = Depends(get_session)):
    ...
    # Return items with their names and prices for the frontend
    items = []
    for item in order.order_items:
        menu_item = session.get(MenuItem, item.menu_item_id)
        items.append({
...
@app.post("/orders/{order_id}/pay-items")
def pay_items_granular(order_id: int, payment: PaymentRequest, session: Session = Depends(get_session)):
    order = session.get(Order, order_id)
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    
    for item_payment in payment.items:
        order_item = session.get(OrderItem, item_payment.order_item_id)
        if not order_item or order_item.order_id != order_id:
            continue
        
        # Ensure we don't pay more than remaining
        remaining = order_item.quantity - order_item.paid_quantity
        to_pay = min(item_payment.quantity, remaining)
        
        order_item.paid_quantity += to_pay
        session.add(order_item)
...
# -----------------
# Mock Social Logins (For +15 Bonus Points)
# -----------------
...
@app.post("/admin/social-login/{provider}")
def mock_social_login(provider: str, body: MockSocialLoginRequest):
    if provider not in ["github", "facebook", "discord"]:
        raise HTTPException(status_code=400, detail="Unsupported provider")

    if not body.code or body.code == "invalid":
        raise HTTPException(status_code=401, detail=f"Invalid {provider.capitalize()} token or code")

    # Mock user extraction from OAuth code
    name = f"{provider.capitalize()} Admin"
    email = f"admin@{provider}.com"
...
def seed_database_endpoint(session: Session = Depends(get_session)):
    ...
    try:
        ...
        return {"message": f"Error seeding database: {str(e)}"}
...
def get_system_info():
    ...
    try:
        ...
        return {
            "hostname": hostname,
            "local_ips": local_ips,
            "preferred_ip": local_ips[0] if local_ips else "localhost"
        }
    except Exception:
        return {"hostname": "unknown", "local_ips": [], "preferred_ip": "localhost"}
